...
//  FoodTruck
//
//  Created by Kevin Stewart on 6/28/20.
...
class OperatorTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = truck.name ?? "Test name"
        cell.detailTextLabel?.text = truck.location ?? "Test location"
...
//    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "DetailSegue" {
            print("Detail segue")
        if let detailVC = segue.destination as? OperatorViewController,
            ...
            print("AddTruck segue")
           if let detailVC = segue.destination as? OperatorViewController {
            print("We are inside where its passing the controller.")
           detailVC.apiController = self.apiController
            }
        }
    }
}
extension OperatorTableViewController: NSFetchedResultsControllerDelegate {
